...
app.post("/posts/add", upload.single("featureImage"), function (req, res, next) {
    ...
    upload(req).then((uploaded) => {
        ...
        blog.addBlog(req.body).then(() => {    //NOTE: req.body will fetch the data from the form, and addMovie will add the data to the temporary array on the page /posts/add, but not save the data to the array in posts.json
            res.redirect("/posts");
        });
    });
});
